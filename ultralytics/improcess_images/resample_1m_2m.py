...
#     base_input_dir = r"D:\windfram&tower\transmission_7bands\images2"         # 原始 YOLO 数据集根目录
...
# ========== 处理单个子文件夹（train/val/test）==========
def downsample_subset_folder(input_folder, output_folder, scale=2.0, resample_alg='average', log_file=None, max_workers=4):
    """处理单个子集文件夹（train/val/test）"""
    os.makedirs(output_folder, exist_ok=True)
    tif_files = [f for f in os.listdir(input_folder) if f.lower().endswith(".tif")]
...
def process_yolo_dataset(base_input_dir, base_output_dir, scale=2.0, resample_alg='average', max_workers=4):
    ...
    input_labels_dir = os.path.join(base_input_dir, "labels")
    ...
    for subset in ["train", "val", "test"]:
        img_input = os.path.join(input_images_dir, subset)
        lbl_input = os.path.join(input_labels_dir, subset)
        img_output = os.path.join(output_images_dir, subset)
        lbl_output = os.path.join(output_labels_dir, subset)

        # 处理图像
        if os.path.exists(img_input):
            log_message(log_file, f"\n开始处理图像: {subset}")
            downsample_subset_folder(img_input, img_output, scale, resample_alg, log_file, max_workers)
        else:
            log_message(log_file, f"警告: 图像文件夹不存在 - {img_input}")
        ...
        if os.path.exists(lbl_input):
            ...
            log_message(log_file, f"警告: 标签文件夹不存在 - {lbl_input}")
